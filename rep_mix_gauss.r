...
for(d in dlist){
	limit=floor(2*d)
	diter=diter+1
	h1<-huge.generator(reps*(n+ntest),d=d,graph="random",prob=0)
	h2<-huge.generator(reps*(n+ntest),d=d,graph="random",prob=0)
	h3<-huge.generator(reps*(n+ntest),d=d,graph="random",prob=0)
	r1=runif(d)
	r2=runif(d)
	r3=runif(d)
	h=cbind(apply(h1$data,1,function(x)x+5*r1),
		apply(h2$data,1,function(x)x+5*r2),
		apply(h3$data,1,function(x)x+5*r3)
	)
	h=t(h)

	h<-scale(h,center=T,scale=T)/8+0.5
	...
	for(r in 1:reps){
		...
		sequ <- exp( seq(from=log(rho_start*.2), to =log(rho_start*1.1),length.out=50 ) )
...
# 		roc[1,diter,r,2]<- 1-sum( (gl$wi[,,glroc][ind]!=0) & (h$theta[ind]==0) ) /(d*(d-1)/2-spar)
		#roc[2,diter,r,1]<-1-sum( (h$theta[ind]!=0) & (glasso_out_relax[,,glrocrelax)][ind]==0) )/spar
		#roc[2,diter,r,2]<- 1- sum( (glasso_out_relax[,,glrocrelax][ind]!=0) & (h$theta[ind]==0) )/(d*(d-1)/2-spar)
...
# 		roc[3,diter,r,2]<-  1-sum( (path[[trwroc]]$tree_obj[]!=0)[ind] & (h$theta[ind]==0) )/(d*(d-1)/2-spar)
		...
		graphs = forestDensityEst(train, 5*d)
...
save.image("~/trw/Experiments/rep_mix_gauss.rdata")